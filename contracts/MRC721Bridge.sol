...
//   event Claim(address indexed user, uint256 txId, uint256 fromChain);
...
//         require(
//           owner == msg.sender || token.isApprovedForAll(owner, msg.sender),
//           "!owner"
//         );
//         token.burnFrom(nftId[index]);
//       }
//     } else {
//       for (uint256 index = 0; index < nftId.length; index++) {
//         address owner = token.ownerOf(nftId[index]);
...
//     IMuonClient.SchnorrSign[] calldata _sigs
//   ) public {
//     claimFor(msg.sender, nftId, nftParams, txParams, _reqId, _sigs);
//   }
...
//     IMRC404 token = IMRC404(tokens[txParams[2]]);
...
//     emit AddToken(tokenAddress, tokenId, _mintable, _transferParameters);
//   }
